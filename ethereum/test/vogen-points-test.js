...
const vogenPoints = artifacts.require('./VOGenPoints.sol')

contract('❍  VOGen Points Tests', async accounts => {

  const gasAmt = 3e6
  const deployerAddress = accounts[0]
  const tokenSymbol = 'VOG'
  const tokenName = 'VOGen Coin'
  const tokenDecimals = 18
  const tokenRate = 1000000000000000000
  const tokenInitialSupply = 1000000000000

  before(async () => (
    { contract } = await vogenPoints.deployed(),
    { methods } = contract,
